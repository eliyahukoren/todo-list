...
import { shouldApplyFilter } from "../../app/helpers";
...
export const todoSlice = createSlice({
  ...
  reducers: {
    ...
    filterTodoAction: (state, action: PayloadAction<ToDoFilter>) => {
      // compl: true, uncomp: true = show all - filter checked
      // compl: false, uncomp: false = show all - no filter default filter status
      // compl: true, uncomp: false = show completed
      // compl: false, uncomp: true = show uncompleted
      // Apply filter section
      if (shouldApplyFilter(action.payload)) {
        let newState;

        if( action.payload.showCompleted ){
          newState = state.todos.filter((item) => item.status !== true);
        }else{
          newState = state.todos.filter((item) => item.status === true);
        }
...
export const { addTodoAction, updateTodoStatusAction, deleteTodoAction, filterTodoAction } = todoSlice.actions;
